...
public class Service
{
    ...
    public PersonDataClass[] GetPersonsInBatchClassesWithPool(int amount)
    {
        PersonDataClass[] result = GetDataArrayPoolClass(amount);
        
        
        for (int i = 0; i < amount; i++)
        {
            result[i] = new PersonDataClass
            {
                Firstname = $"Firstname{i}",
                Lastname = $"Lastname{i}",
                BirthDate = DateTime.Now.AddYears(-20).AddDays(i),
    ...
    public PersonDataStruct[] GetPersonsInBatchStructs_withArrayPool(int amount)
    {
        PersonDataStruct[] result = GetDataArrayPoolStructs(amount);
        for (int i = 0; i < amount; i++)
        {
    ...
    internal PersonDataClass[] GetDataArrayPoolClass(int amount)
    {
        PersonDataClass[] result = ArrayPool<PersonDataClass>.Shared.
            Rent(amount);
        return result;
    }
...
public class Benchy
{
    ...
    [Benchmark]
    public List<string> FilterPeopleWithinLocation_Classes_withArrayPool()
    {
        var locationLat = Latitude;
        var locationLon = Longitude;
        var now = DateTime.Now;

        var input = service.GetPersonsInBatchClassesWithPool(Amount);
    ...
    [Benchmark]
    public List<string> FilterPeopleWithinLocation_Structs_WithArrayPool()
    {
        var locationLat = Latitude;
        var locationLon = Longitude;
        var now = DateTime.Now;

        var input = service.GetPersonsInBatchStructs_withArrayPool(Amount); // Преобразуем список в массив
        var result = new List<string>(input.Length);
